...
def criar_aba1(notebook, criar_pergunta):
  ...
  assunto_entry = tk.Entry(tab1, width=50)
  ...
  # linha 4
  pergunta_exemplo_text = tk.Text(tab1)
  pergunta_exemplo_text.configure(height=15)
  pergunta_exemplo_text.grid(row=4, column=0, columnspan=4, sticky="WE", padx=5, pady=5)
  ...
  def executar():
    ...
    resposta = criar_pergunta(assunto_entry.get(), tipo_var.get(), dificuldade_var.get(), pergunta_exemplo_text.get("1.0", "end"))
...
def criar_aba2(notebook, responder_pergunta):
  ...
  pergunta_text.configure(height=15)
  ...
  ok_button.grid(row=7, column=0, sticky="WE", padx=5, pady=5)
